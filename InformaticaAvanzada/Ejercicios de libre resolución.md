...
public class Transportes {
    private ArrayList<Vehiculo> moviles;

    public Transportes () {
        moviles = new ArrayList<>();
    }

    public void addMovil (Vehiculo movil) {
        // TODO implementar
        this.moviles.add(movil);
    }

    public ArrayList<Vehiculo> getAereos () {
        // TODO implementar
        ArrayList<Vehiculo> aereos= new ArrayList<>();
        for(Vehiculo v : moviles ) {
		    if(v.getMedio() == Tipo.AEREO) {
				aereos.add(v);    
			}
        } 
        return aereos;
...
public class Triciclo extends Vehiculo {
    ...
    String cadena = "Triciclo de "+ getPropietario() + " (" + super.toString() + ")";
    return cadena;
    }
}
```
